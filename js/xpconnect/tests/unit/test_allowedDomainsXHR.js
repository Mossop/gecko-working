...
function finishIfDone() {
  if (++httpServersClosed == 3) do_test_finished();
}
...
function run_test() {
  ...
  var res = Cu.evalInSandbox(
    'var sync = createXHR("4444/simple"); sync.send(null); sync',
    sb
  );
  ...
  try {
    Cu.evalInSandbox("var createXHR = " + createXHR.toString(), sb);
    var res = Cu.evalInSandbox(
      'var sync = createXHR("4445/negative"); sync.send(null); sync',
      sb
    );
    Assert.equal(
      false,
      true,
      "XHR created from sandbox should not have chrome caps"
    );
  } catch (e) {
    Assert.ok(true);
  }
  ...
  Cu.evalInSandbox("var createXHR = " + createXHR.toString(), sb);
  var res = Cu.evalInSandbox(
    'var sync = createXHR("4444/redirect"); sync.send(null); sync',
    sb
  );
  Assert.ok(checkResults(res));
  ...
  function changeListener(event) {
    if (checkResults(async)) finish();
  }

  var async = Cu.evalInSandbox(
    'var async = createXHR("4444/simple", true);' +
      'async.addEventListener("readystatechange", ' +
      changeListener.toString() +
      ", false);" +
      "async",
    sb
  );
