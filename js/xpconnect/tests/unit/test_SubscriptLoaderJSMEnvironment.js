...
const a = ChromeUtils.import(
  "resource://test/environment_loadscript.jsm",
  tgt_load
);
const b = ChromeUtils.import(
  "resource://test/environment_checkscript.jsm",
  tgt_check
);
...
for (var name of ["vu", "vq", "vl", "gt", "ed", "ei", "fo", "fi", "fd"])
  if (tgt_load.target.hasOwnProperty(name)) tgt_subscript_bound += name + ",";
...
Assert.equal(
  tgt_load.bound,
  "vu,ei,fo,fi,",
  "Should have expected module binding set"
);
Assert.equal(
  tgt_subscript_bound,
  "vq,gt,ed,fd,",
  "Should have expected subscript binding set"
);
...
if (isShared) {
  ...
  Assert.equal(
    tgt_check.bound,
    "ei,fo,",
    "Modules should have no shared non-eval bindings"
  );
} else {
