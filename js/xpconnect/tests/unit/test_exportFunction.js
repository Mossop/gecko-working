function run_test() {
  var epsb = new Cu.Sandbox(["http://example.com", "http://example.org"], {
    wantExportHelpers: true,
  });
  var subsb = new Cu.Sandbox("http://example.com", {
    wantGlobalProperties: ["XMLHttpRequest"],
  });
  var subsb2 = new Cu.Sandbox("http://example.com", {
    wantGlobalProperties: ["XMLHttpRequest"],
  });
  ...
  Cu.evalInSandbox(
    "(" +
      function () {
        ...
        exportFunction(funToExport, subsb, {
          defineAs: "imported",
          allowCallbacks: true,
        });
        exportFunction(x => x, subsb, {
          defineAs: "echoAllowXO",
          allowCallbacks: true,
  ...
  try {
    Assert.equal(
      Cu.evalInSandbox("echoAllowXO(xoNative)", subsb),
      subsb.xoNative
    );
    Assert.ok(true);
  } catch (e) {
  ...
  Cu.evalInSandbox(
    "(" +
      function () {
        var newContentObject = createObjectIn(subsb, {
          defineAs: "importedObject",
        });
        exportFunction(funToExport, newContentObject, {
          defineAs: "privMethod",
        });
      }.toSource() +
      ")()",
  ...
  var newContentObject = Cu.createObjectIn(subsb, {
    defineAs: "importedObject2",
  });
  var wasCalled = false;
  Cu.exportFunction(
    function (arg) {
      wasCalled = arg.wasCalled;
    },
    newContentObject,
    { defineAs: "privMethod" }
  );
