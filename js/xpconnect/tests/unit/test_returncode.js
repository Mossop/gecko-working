...
function test_simple(contractID) {
  ...
  Assert.equal(
    result,
    Cr.NS_ERROR_XPC_JAVASCRIPT_ERROR_WITH_DETAILS,
  ...
  result = parent.callChild(
    Ci.nsIXPCTestReturnCodeChild.CHILD_SHOULD_RETURN_SUCCESS
  );
  ...
  Assert.deepEqual(
    getConsoleMessages(),
    [],
    "no messages reported with .returnCode"
  );
}
...
function test_nested(contractID) {
  ...
  result = parent.callChild(
    Ci.nsIXPCTestReturnCodeChild.CHILD_SHOULD_NEST_RESULTCODES
  );
  Assert.equal(
    result,
