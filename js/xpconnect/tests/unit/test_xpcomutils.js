...
add_test(function test_defineLazyServiceGetter() {
  ...
  let service = Cc["@mozilla.org/consoleservice;1"].getService(
    Ci.nsIConsoleService
  );

  // Check that the lazy service getter and the actual service have the same
  // properties.
  for (let prop in obj.service) Assert.ok(prop in service);
  for (let prop in service) Assert.ok(prop in obj.service);
  run_next_test();
});
...
add_test(function test_defineLazyPreferenceGetter() {
  ...
  equal(
    obj.pref,
    "defaultValue",
    "Should return the default value before pref is set"
  );
  ...
  equal(
    obj.pref,
    "currentValue",
    "Should return the current value on initial read when pref is already set"
  );
  ...
  equal(
    obj.pref,
    "newValue",
    "Should return new value after preference change"
  );
  ...
  equal(
    obj.pref,
    "currentValue",
    "Should return new value after second preference change"
  );
  ...
  equal(
    obj.pref,
    "defaultValue",
    "Should return default value after pref is reset"
  );

  obj = {};
  XPCOMUtils.defineLazyPreferenceGetter(obj, "pref", PREF, "a,b", null, value =>
    value.split(",")
  );
...
add_test(function test_categoryRegistration() {
  ...
  const XULAPPINFO_CID = Components.ID(
    "{fc937916-656b-4fb3-a395-8c63569e27a8}"
  );
  ...
  const expectedEntries = new Map([
    ...
    [
      ...
      "@unit.test.com/cat-app-registered-component;1",
    ],
  ]);
...
add_test(function test_categoryBackgroundTaskRegistration() {
  ...
  let expectedEntriesList = [
    ...
    [
      ...
      "@unit.test.com/cat1-backgroundtask-notregistered-component;1",
    ],
  ];
  if (!AppConstants.MOZ_BACKGROUNDTASKS) {
    expectedEntriesList.push(
      ...[
        [
          "Cat1BackgroundTaskRegisteredComponent",
          "@unit.test.com/cat1-backgroundtask-registered-component;1",
        ],
        [
          "Cat1BackgroundTaskAlwaysRegisteredComponent",
          "@unit.test.com/cat1-backgroundtask-alwaysregistered-component;1",
        ],
      ]
    );
  }
  ...
  for (let { entry, value } of Services.catMan.enumerateCategory(
    ...
    Assert.equal(
      value,
      expectedEntries.get(entry),
      "Verify that the value is correct in the expected entries."
    );
    expectedEntries.delete(entry);
  }
  Assert.deepEqual(
...
add_test(function test_generateSingletonFactory() {
  ...
  const XPCCOMPONENT_CID = Components.ID(
    "{31031c36-5e29-4dd9-9045-333a5d719a3e}"
  );
