...
function run_test() {
  ...
  for (var i = 0; i < 8; ++i) new Uint8Array(sb.ab)[i] = i * 10;
  sb.ta = [];
  ...
  checkThrows(
    "Object.prototype.__lookupSetter__('__proto__').call(obj, {});",
    sb
  );
  sb.re = /f/;
  ...
  checkThrows(
    "ArrayBuffer.prototype.__lookupGetter__('byteLength').call(ab);",
    sb
  );
  ...
  function testForTypedArray(t) {
    ...
    checkThrows(
      "(new this[currName]).__lookupGetter__('length').call(curr)",
      sb
    );
    checkThrows(
      "(new this[currName]).__lookupGetter__('buffer').call(curr)",
      sb
    );
    checkThrows(
      "(new this[currName]).__lookupGetter__('byteOffset').call(curr)",
      sb
    );
    checkThrows(
      "(new this[currName]).__lookupGetter__('byteLength').call(curr)",
      sb
    );
  }
