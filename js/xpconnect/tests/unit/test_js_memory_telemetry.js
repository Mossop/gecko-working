...
add_task(function test_compartment_realm_counts() {
  ...
  let snapshot1 = Services.telemetry.getSnapshotForHistograms(
    "main",
    true
  ).parent;
  ...
  Assert.ok(
    snapshot1[realmsSystem].sum <= snapshot1[compsSystem].sum,
    "Number of system compartments can't exceed number of system realms"
  );
  Assert.ok(
    snapshot1[realmsUser].sum <= snapshot1[compsUser].sum,
    "Number of user compartments can't exceed number of user realms"
  );
  Assert.ok(
    snapshot1[realmsSystem].sum > 0,
    "There must be at least one system realm"
  );
  ...
  Services.telemetry.gatherMemory();
  let snapshot2 = Services.telemetry.getSnapshotForHistograms(
    "main",
    true
  ).parent;
  ...
  for (let k of [realmsSystem, realmsUser, compsUser]) {
    Assert.ok(
      snapshot2[k].sum > snapshot1[k].sum,
      "There must be more compartments/realms now: " + k
    );
  }

  Assert.ok(
    snapshot2[realmsSystem].sum > snapshot2[compsSystem].sum,
