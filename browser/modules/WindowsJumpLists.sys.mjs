...
var Builder = class {
   ...
   *
   * @param {boolean} isShutdown true if this update is the final update before shutdown.
   ...
   */
  async buildList(isShutdown = false) {
    ...
    // shutdown update
    if (!isShutdown && (this._isBuilding || this._shuttingDown)) {
      return;
    }
    ...
    try {
      ...
      if (isShutdown || !this._shuttingDown) {
  ...
  async _fetchFrequent(isShutdown, removedURLs) {
    ...
    for (let row of rows) {
      ...
      // nsIJumpListBuilder.checkForRemovals returns the first argument so that
      // must be the url.
      let args = `${row.getResultByName("url")} -profile ${profilePath}`;
